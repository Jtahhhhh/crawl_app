...
class ChapDetail extends StatelessWidget {
  ...
  ChapDetail({ required this.index, this.isShowFromDetail = true});

  @override
  Widget build(BuildContext context) {
    final comicLink =controller.comic.value.linkDetail.split(" ")[0];
    int chapterNumber;
    try {
      chapterNumber = int.parse(controller.chapterList[index].linkDetail);
    } catch (e) {
          ...
          children: [
            ...
            Obx(() {
              if (controller.isImgLoading.value) {
                ...
                print("img list: ${controller.listImg}");
                String htmlContent = controller.listImg.map((chap) {
                  return '<img decoding="async" loading="lazy" src="$chap" class="img-fluid" alt="${controller.comic.value.title}" />';
                }).join('');
                        ...
                        if (!controller.isReversed.value) {
                          ...
                          if (index > 0) {
                            ...
                            controller.fetchChapter(
                                comicLink, int.parse(controller.chapterList[index].linkDetail));}
                        } else {
                          // Reversed navigation
                    ...
                    DropdownButton<int>(
                      ...
                      onChanged: (int? newIndex) async {
                        if (newIndex != null && newIndex != index) {
                          ...
                          await controller.fetchChapter(
                              comicLink, int.parse(controller.chapterList[newIndex].linkDetail));
                      ...
                      items: controller.chapterList.asMap().entries.map<DropdownMenuItem<int>>((entry) {
                        ...
                        return DropdownMenuItem<int>(
                          ...
                          child: Text('Chapter ${controller.chapterList[idx].name}', style: TextStyle(color: Colors.white)),
                        );
                      }).toList(),
                    ),
                    ElevatedButton(
